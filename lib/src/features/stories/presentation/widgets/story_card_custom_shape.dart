...
class SlantedShapeClipper extends CustomClipper<Path> {
  ...
  final double radius;
  final double topSlantHeight;

  SlantedShapeClipper({
    required this.isLeft,
    required this.slantHeight,
    required this.radius,
    this.topSlantHeight = 25.0,
  });
  ...
  Path getClip(Size size) {
    ...
    if (isLeft) {
      ...
      path.moveTo(0, topSlantHeight + radius);
      path.quadraticBezierTo(0, topSlantHeight, radius, topSlantHeight);
      ...
      // উপরের ডান কোণা
      path.lineTo(size.width - radius, topSlantHeight);
      path.quadraticBezierTo(
        size.width,
        topSlantHeight,
        size.width,
        topSlantHeight + radius,
      );
    }
    ...
    if (isLeft) {
      ...
      path.quadraticBezierTo(
        ...
        size.height - slantHeight,
        0,
        size.height - slantHeight - radius,
      );
    }
